...
MAX_RETRIES = 15  # Maximum number of reconnection attempts
...
def capture_stream(rtsp_url):
    global cap, stream_halt, last_frame_time, log_warning_flag, stream_stuck, show_stream
    ...
    while True:
        ...
        try:
            if not stream_halt:
                ret, frame = cap.read()
                if show_stream:  # Check the show_stream flag
                    # Update the GUI
            ...
            if ret and frame is not None and frame.size > 0:
                last_frame_time = time.time()
                frame_queue.put(frame)  # Put frame into queue
                log_warning_flag = False
                stream_stuck = False
                ...
                if not cap.isOpened() and not log_warning_flag:
                    logging.warning("capture_stream: VideoCapture is not open. unable to access video stream")
                    log_warning_flag = True
                if not log_warning_flag:
                    logging.warning("capture_stream: VideoCapture is open but Failed to grab frame/invalid frame")
                    log_warning_flag = True
                # time.sleep(2)

        except Exception as e:
            logging.warning("capture_stream: Exception occured while capturing frame: "+str(e))


def monitor_stream(rtsp_url):
    global last_frame_time, cap, reconnect_attempts, stream_halt, show_stream
        ...
        try:
            ...
            if time_since_last_frame > STUCK_THRESHOLD or stream_stuck:
                ...
                if reconnect_attempts < MAX_RETRIES:
                    logging.warning("mon_stream: Stream stuck. Attempting to reconnect...")
                    try:
                        if cap_lock.acquire(timeout=1):  # Try to acquire the lock with a timeout of 5 seconds
                            cap.release()
                            time.sleep(2)
                            cap = cv2.VideoCapture(rtsp_url)
                            time.sleep(2)
                            ret, frame = cap.read()
                        if ret and frame is not None and frame.size > 0:
                            logging.warning("mon_stream: Stream reconnected")
                            stream_halt = False
                            show_stream = True
...
def process_frame():
    global last_motion_time, recording, recording_thread_active, frames_queue, motion_status, show_stream
    while True:
        time.sleep(0.01)
        try:
            ...
            if recording:
                ...
                if time.time() - last_motion_time > record_duration:# ending the recording
                    recording = False
                    motion_status = False
...
# process_thread = Thread(target=process_frame)
